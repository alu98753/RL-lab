...
class CarRacingTD3Agent(TD3BaseAgent):
	def __init__(self, config):
		...
		noise_type = config.get("noise_type", "uniform")
		...
		if noise_type == "OUNoise":
			...
			self.noise = UniformNoiseGenerator(low=-1.0, high=1.0, size=self.env.action_space.shape[0])
		elif noise_type == "perlin":
			self.noise = PerlinNoiseGenerator(size=self.env.action_space.shape[0], scale=0.1)
		elif noise_type == "sinusoidal":
			self.noise = SinusoidalNoiseGenerator(amplitude=0.5, frequency=0.01, size=self.env.action_space.shape[0])
		else:
...
class PerlinNoiseGenerator:
    ...
    def generate(self):
        self.t += self.scale
        return np.array([pnoise1(self.t + i * self.scale) for i in range(self.size)])

class SinusoidalNoiseGenerator:
    def __init__(self, amplitude, frequency, size):
    ...
    def generate(self):
        self.t += 1
        return self.amplitude * np.sin(self.frequency * self.t + np.arange(self.size))
