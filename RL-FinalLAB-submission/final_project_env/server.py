...
def get_img_views():
    ...
    img1 = env.force_render(render_mode='rgb_array_higher_birds_eye', width=540, height=540,
                                position=np.array([4.89, -9.30, -3.42]), fov=120)
    img2 = env.force_render(render_mode='rgb_array_birds_eye', width=270, height=270)
    img3 = env.force_render(render_mode='rgb_array_follow', width=128, height=128)
...
if __name__ == '__main__':
    ...
    print("Has force_render:", hasattr(env, "force_render"))

    obs, info = env.reset()
    #
    app.run(debug=False, host='0.0.0.0', port=port)

# python server.py --port 33333 --sid tester --scenario austria_competition
# python server.py --port 33333 --sid tester --scenario circle_cw_competition_collisionStop
